...
results$countycount <- sapply(results$final_roi, function(x) sum(x == "1"))


png(filename = paste0("output/", job_name, "/", job_name, "_roisize.png"))
print(ggplot(results, aes(x = dev_value, y = p_cutoff, fill = log(countycount))) +
        scale_fill_gradient(low = "blue", high = "yellow") +
        geom_tile() +
        labs(title = "County count"))
dev.off()
