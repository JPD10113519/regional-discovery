...
## new function for converting task id to a binary string for swiss cantons
task_id_to_binary <- function(task_id, n_bits = 26) {
  as.integer(intToBits(task_id)[n_bits:1])
}

final_features_to_index <- function(features_list) {
  fullfeatures <- c("PctCatholic", "PctDE", "PctMigrationshg", "PctUnemployed", "PctFarRight")
  bits <- as.integer(fullfeatures %in% features_list)
  features_id <- strtoi(paste0(bits, collapse = ""), base = 2)
  return(features_id)
}
...
run_iteration <- function(target_dev, p_cutoff, ROI, max_iterations=20) {
  iterations <- 0 ## at the start, no iterations
  ...
  while(iterations < max_iterations) {
    ...
    dev_model <- glmnet(x = PREDS, y = current_roi, family = "binomial", 
                        alpha = 1, standardize = TRUE)  # Consider if you need standardization
    ...
    dev_predictions <- predict(dev_model, newx = PREDS, 
                               ...
                               type = "response")
    #cat(dev_predictions[ids]) ## testing line, see ../presentation/idek.R
  ...
  vars <- "N/A: No Model Fit"
  if(!is.null(dev_model)) {
  ...
  list(iterations = iterations,
       ...
       laststep_preds = dev_predictions,
...
genmap <- function(bin=NA, int = NA, listcol=NA) {
  ...
  map$color <- rgb(r=0.1, g=0.1, b=0.9, alpha=(4*ROI/5))
  plotmap <- ggplot(data=map, aes(fill=color)) + geom_sf(color=rgb(0.2,0.2,0.2),linewidth=0.8) + scale_fill_identity() +
    theme_void()
  print(plotmap)
}
